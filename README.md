...
## 6. 구현 제외
- **iterator 클래스**  
    iterator을 이용한 트리 순회의 속도도 STL과 비교할 수 있으면 좋겠지만, 프로젝트 규모를 제한하고자 삽입-검색-삭제-소멸의 시간만 비교하기로 하여 구현 대상에서 제외하였습니다.

<br>

- **키 타입 템플릿, emplace 메소드**  
   주어진 타입의 비교자 정의 여부 파악, 가변 인자 템플릿 등을 이용하려면 C++ 기능을 추가적으로 학습하여야 하기에 프로젝트 규모를 제한하고자 제외하였습니다.
...
- **세부적인 최적화**  
    삽입 메소드에서 단순한 데이터 타입 매개변수에 대해선 참조 방식이 아니라 값전달 방식을 사용하도록 하는 것, 그리고 스택 자료구조의 내부 어레이가 확장될 때 이동 연산의 예외 안전성 여부를 확인하여 가능하면 복사 대신 이동을 사용하도록 하는 것들을 구현하려면 C++ 기능을 추가적으로 학습하여야 하기에 프로젝트 규모를 제한하고자 제외하였습니다.

<br>

- **노드의 부모 포인터**  
    스택을 이용하지 않고 순회 및 조정을 할 수 있도록 하면 속도와 예외 안전성을 높일 수 있으나, 이를 반영하기 위해선 네 종류 트리의 주요 메소드들을 모두 수정해야 하기에 프로젝트 규모를 제한하고자 제외하였습니다.
