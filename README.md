...
부모노드의 키(key)는 좌측 자식 서브 트리의 키보다는 크고, 우측 서브 트리의 키보다는 작아야 한다는 조건을 통해 빠르게(평균적으로 O(log(N))) 특정 키의 노드를 찾을 수 있습니다 [1].  
...
현재 구현은 검색의 대상이 된 노드의 높이를 1~2 만큼만 올리도록 구현이 되어 있어 수정하고 있습니다.
...
### 1.4. 레드-블랙 트리
레드-블랙 트리(Red-Black Tree)는 균형이 지켜지는 2-3-4 트리의 개념을, 이진 탐색 트리로 모방 구현한 트리입니다. 

이진 탐색 트리의 색깔이 부여된 노드들로 2-3-4 트리에서 여러개의 데이터가 들어간 노드들을 모방하여, 제한적이지만 트리의 균형을 유지하려고 합니다 [1].

현재 구현은 다량의 삽입 및 삭제 테스트 결과, 노드나 서브 트리가 유실되는 것으로 추측되는 오류가 발생하고 있어 디버깅 중입니다.
...
3. Visual Studio가 실행되면 우측의 솔루션 탐색기 바를 확인합니다.  
(해당 솔루션 탐색기 바가 없을 시 상단의 보기(V) 바에서 솔루션 탐색기(P)를 누릅니다.)

4. 해당 솔루션 탐색기 바에서 실행해보고 싶은 트리 프로젝트를 우클릭합니다.

5. 우클릭 시 나오는 메뉴에서 시작 프로젝트로 설정을 누르거나, 단축키 `A`를 누릅니다.

6. 기능 테스트를 실행하려면, Visual Studio 상단의 구성(configuration)을 `Debug` `x64`로 설정하고, 디버깅 시작키인 F5를 누릅니다.  
실행 결과가 검은 명령 프롬프트에 출력됩니다. 또한 메모리 누수가 존재한다면 Visual Studio 하단의 출력(output) 창에 누수 정보가 출력됩니다.
...
- 따라서 BST의 삽입과 검색 메소드가 상대적으로 10% 정도 느린 속도는 세부적인 최적화가 부족하기 때문으로 추측합니다.

- 또한 BST의 삭제 메소드가 느린 이유는 인라이닝되지 못한 것 때문으로 추측합니다.
...
- 그럼에도 앞선 랜덤 워크로드 테스트에 비해서 BST의 삽입, 검색, 삭제 메소드가 더 빨라진 이유는 캐시 히트율이 높아졌기 때문으로 추측합니다.

- BST와 std::map의 속도 차이가 벌어진 이유는 설계와 구현상에서 캐시 히트율 고려에 대한 수준 차이 때문으로 추측합니다.
...
- 삽입 메소드와 검색 메소드는 리스트와 같은 모양이 되어서 전체 O(N^2)의 시간 복잡도 때문에 시간 초과(=50초 이상)가 납니다.

- 시간 초과로 삽입이 덜 이루어졌으므로, 이후의 '삭제', '소멸'의 측정 시간은 신뢰할 수 없습니다.

- BST와 달리 std::map은 균형을 유지하는 트리이기 때문에 선형 워크로드에 대해서도 전체 O(NlogN)의 시간 복잡도를 가져 매우 빠른 속도를 보입니다.

- 게다가 선형 워크로드가 가진 지역성으로 캐시 히트율이 증가해 1번 랜덤 워크로드 테스트보다 훨씬 빠른 속도를 보이는 것으로 추측합니다.
...
앞선 검색 메소드에서 splay 조정이 트리를 편향시켜서 속도를 느리게 하는 것으로 추측합니다.
...
- SplayTree에서 삽입의 속도보다 검색의 속도가 느리다는 점, 그리고 검색 이후 삭제 메소드에서도 속도가 느려졌다는 점을 고려하면, 앞선 랜덤 워크로드 테스트와 마찬가지로 검색 메소드 중 수행하는 splay 조정이 트리를 편향시키는 것으로 추측합니다.
...
- 반대로 삭제 메소드는 검색으로 인해 트리가 조정되어 삭제할 노드가 루트 노드에 위치하지 않게 되므로 검색보다 느린 것으로 추측합니다.
...
- 앞선 선형 증가 워크로드와 달리 본 워크로드에서 삭제 메소드가 시간 초과(=50초 이상)를 내는 것은 아직 원인을 파악하지 못하였습니다.
...
- AVLTree의 삽입 메소드와 삭제 메소드가 상대적으로 느린 이유는 해당 메소드에서 균형 유지 로직이 비효율적인 것 때문으로 추측합니다.
...
- 앞선 BST 테스트와 비슷한 결과가 나온 것으로 보아, 캐시 히트율 차이도 std::map과의 속도 차이의 원인 중 하나로 추측합니다.
...
- 삭제가 10% 정도 수행되고 있을 때 "Cannot Remove! Cannot find such target node!" 오류가 출력됩니다. 앞선 메소드들의 노드 조정 과정에서 일부 노드나 서브 트리가 소실되는 로직 오류로 추측하고서 추후 디버깅할 예정입니다.
...
- **속도 측정 반복 통계**
    - 주어진 횟수로 속도 측정 테스트를 반복하고, 평균과 분산 값을 구할 수 있도록 하기
...
## 6. 구현 제외
- **iterator 클래스**  
    iterator을 이용한 트리 순회의 속도를 STL과 비교할 수 있으면 좋겠지만, 프로젝트 규모를 제한하고자 삽입-검색-삭제-소멸의 시간만 비교하기로 제한하였습니다.

<br>

- **키 타입 템플릿, emplace 메소드**  
   주어진 타입의 비교자 정의 여부 파악, 가변 인자 템플릿을 이용한 기능들은 프로젝트 규모를 제한하고자 제외하였습니다.

<br>

- **메모리 풀**  
    메모리 풀로부터 연속된 공간 속에서 노드 공간을 배정받을 수 있도록 하면 동적 할당 횟수를 줄이고 캐시 히트율 상승을 기대할 수 있으나, 프로젝트 규모를 제한하고자 구현 대상에서 제외하였습니다.

<br>

- **세부적인 최적화**  
    삽입 메소드에서 단순한 데이터 타입 매개변수에 대해선 참조 방식이 아니라 값전달 방식을 사용하도록 하는 것, 그리고 스택 자료구조의 내부 어레이가 확장될 때 이동 연산의 예외 안전성 여부를 확인하여 가능하면 복사 대신 이동을 사용하도록 하는 것은 프로젝트 규모를 제한하고자 제외하였습니다.
